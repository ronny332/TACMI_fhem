...
const cmiUdpServer = (() => {
  ...
  server.on('message', (msg, rinfo) => {
    ...
    if (config.cmi.udp.server._debug) {
      ...
      msg.forEach((val) => {
        msgBin.push(Number(val).toString(2).padStart(8, '0'));
        msgHex.push(Number(val).toString(16).padStart(2, '0'));
      });
  ...
  // bind server
  server.bind(config.cmi.udp.server.port);
