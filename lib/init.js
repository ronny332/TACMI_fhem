...
const init = (() => {
  ...
  // config injection
  const injections = ['fhem.telnet', 'cmi.knots', '_global'];

  let file,
    pathCurrent = config._global.path;
  ...
  if (pathCurrent !== config._global.path) {
    if (config.init._debug) {
      console.info('(dbg) config path changed, checking new path again');
    }
    checkConfigFolder();
  }
