...
var cmiData = (() => {
  const config = require(__base + 'lib/config'),
    dataTypes = require(__base + 'lib/cmi.data.types'),
  ...
  const _convertDigitalValueStringToBoolean = (type) => {
    ...
    const types = Object.keys(dataTypes.types);
    ...
    switch (type) {
      case 'A':
        ...
        for (let i = start; i < start + 4; i++) {
          if (data[type][knot][i].date > 0) {
            if (config.cmi.knots[knot].items.hasOwnProperty(no) && config.cmi.knots[knot].items[no].hasOwnProperty('unit')) {
              unit = dataUnits.units[config.cmi.knots[knot].items[no].unit] | 1;
            }

            message.setValue(type, i, data[type][knot][i].value, unit, config.cmi.knots[knot].items[i].decimals);
          }
        }
      ...
      case 'D':
        ...
        for (let i = start; i < start + 16; i++) {
          if (data[type][knot][i].date > 0) {
            message.setValue(type, i, !!data[type][knot][i].value, 1);
          }
        }
        ...
        for (let i = 1; i < 5; i++) {
          ...
          if (config.cmi.knots.hasOwnProperty(knot) &&
            ...
            if (config.cmi.knots[knot].items[no].unit === 'auto') {
              unit = dataUnits.ids[units[i]];
            }
            else {
              // custom unit by id
              if (dataUnits.ids.hasOwnProperty(config.cmi.knots[knot].items[no].unit)) {
                unit = dataUnits.ids[config.cmi.knots[knot].items[no].unit];
              }
              // custom unit by unit
              else if (dataUnits.units.hasOwnProperty(config.cmi.knots[knot].items[no].unit)) {
                unit = config.cmi.knots[knot].items[no].unit;
              }
              // invalid configuration for unit
              else {
                unit = dataUnits.ids[0];
              }
            }
          }
          else {
      ...
      case 'D':
        ...
        for (let i = 1; i < 17; i++) {
          ...
          if (config.cmi.knots.hasOwnProperty(knot) &&
            ...
            // custom type by id
            if (dataTypes.ids.hasOwnProperty(config.cmi.knots[knot].items[no].format)) {
              value = dataTypes.getValue(!!values[i], dataTypes.ids[config.cmi.knots[knot].items[no].format]);
            }
            // custom type by type
            else if (dataTypes.types.hasOwnProperty(config.cmi.knots[knot].items[no].format) || config.cmi.knots[knot].items[no].format.indexOf('/') !== -1) {
              value = dataTypes.getValue(!!values[i], config.cmi.knots[knot].items[no].format);
            }
