  ...
  const init = (type, cb) => {
    if (type === 'fhem') {
      getConnection((client) => {
        ...
        commands.push(`attr ${config.fhem.telnet.client.device} event-on-change-reading .*`);

        if (config.fhem.telnet.client.hasOwnProperty('room') && config.fhem.telnet.client.room.trim().length > 0) {
          commands.push(`attr ${config.fhem.telnet.client.device} room ${config.fhem.telnet.client.room.trim()}`);
        }
        ...
        commands.push(`define n_${config.fhem.telnet.client.device} notify ${config.fhem.telnet.client.device}:(\\+.*)\\s(.*) {\\
  use Net::Telnet();;\\
\\
...
`);
        ...
        if (config.fhem.telnet.client.hasOwnProperty('room') && config.fhem.telnet.client.room.trim().length > 0) {
          commands.push(`attr n_${config.fhem.telnet.client.device} room ${config.fhem.telnet.client.room.trim()}`);
        }
