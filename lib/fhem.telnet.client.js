  ...
  const init = (type, cb) => {
    if (type === 'fhem') {
      getConnection((client) => {
        const commands = [],
          fhemDevice = _cleanName(config.fhem.telnet.client.device.trim()),
        ...
        if (config.fhem.telnet.client.hasOwnProperty('room') && config.fhem.telnet.client.room.trim().length > 0) {
          commands.push(`attr ${fhemDevice} room ${_cleanName(config.fhem.telnet.client.room.trim())}`);
        }

        if (config.fhem.telnet.client.hasOwnProperty('alias') && config.fhem.telnet.client.alias.trim().length > 0) {
          commands.push(`attr ${fhemDevice} alias ${_cleanName(config.fhem.telnet.client.alias.trim())}`);
        }
                  ...
                  switch (config.cmi.knots[knot].type) {
                    case 'A':
                      if (config.cmi.knots[knot].items[key].hasOwnProperty('decimals') &&
                        ...
                        readings.push(`setreading ${fhemDevice} ${prefix}${_cleanName(config.cmi.knots[knot].items[key].name)} ${value}`);
                    ...
                    case 'D':
                      if (config.cmi.knots[knot].items[key].hasOwnProperty('format')) {
                        ...
                        if (format.length === 2) {
                          readings.push(`setreading ${fhemDevice} ${prefix}${_cleanName(config.cmi.knots[knot].items[key].name)} ${format[1]}`);
      ...
      getConnection((client) => {
        ...
        update.forEach((d) => {
          if (type === 'A') {
            ...
            commands.push(`setreading ${fhemDevice} ${_cleanName(data[type][d.knot][d.no].name)} ${data[type][d.knot][d.no].value.toFixed(decimals)}`);
          }
          else {
            commands.push(`setreading ${fhemDevice} ${_cleanName(data[type][d.knot][d.no].name)} ${data[type][d.knot][d.no].value}`);
          }
        });
