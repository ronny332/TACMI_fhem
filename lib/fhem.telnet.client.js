...
const fhemTelnetClient = ((data) => {
  ...
  data.on('update', (type, update) => {
    setImmediate(() => {
      ...
      const client = net.connect({host: '10.0.0.23', port: 7072, timeout: 2500}, () => {
        ...
        update.forEach((d) => {
          if (type === 'A') {
            ...
            cmd.push(`setreading ${config.fhem.telnet.client.device} ${_cleanName(data[type][d.knot][d.no].name)} ${data[type][d.knot][d.no].value.toFixed(decimals)}`);
          }
          else {
            cmd.push(`setreading ${config.fhem.telnet.client.device} ${_cleanName(data[type][d.knot][d.no].name)} ${data[type][d.knot][d.no].value}`);
          }
        });
        ...
        if (config.fhem.telnet.client._debug) {
          console.info(`(dbg) sending commands to ${config.fhem.telnet.client.host}:${config.fhem.telnet.client.port}: "${cmd.join(';')}"`);
        }
      ...
      // client.on('end', () => {
      //   if (config.fhem.telnet.client._debug) {
      //     console.log(`(dbg) closing connection to ${config.fhem.telnet.client.host}:${config.fhem.telnet.client.port}`);
      //   }
      // });
